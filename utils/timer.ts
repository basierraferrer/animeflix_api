...
export const getTimerArray = (textHTML:string, title: string) => {
    ...
    if (title === PLAYER.SW) {
      ...
      timerArray = $cheerio(timerSelector).attr('aria-valuetext')?.replace('0 seconds of ', '').trim().split(',') as string[];
    }
...
export const getTimerMilliseconds = (textHTML: string, title: string) => {
    ...
    const totalMilliseconds:number = timerArray.reduce((total:number, item: string) => {
